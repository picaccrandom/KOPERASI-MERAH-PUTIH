                    ...
                    <div class="card-body p-4">
                        <div class="row mb-3">
                            <div class="col-md-12 position-relative">
                                <label class="form-label fw-bold small text-muted text-uppercase text-danger">Cari Barang
                                    (Barcode / Nama)</label>
                ...
                <div class="card payment-card shadow-sm border-0">
                    ...
                    <div class="card-body p-4">
                        <form id="form-transaksi">
                            ...
                            <div class="mb-4">
                                <label class="form-label fw-bold small uppercase text-muted mb-2 d-block text-center">Status
                                    Pelanggan</label>
                                <div class="d-flex justify-content-around status-selection shadow-sm">
                                    <label class="text-center cursor-pointer mb-0 flex-grow-1 mx-1">
                                        <input type="radio" name="kat_radio" id="kategori_no" value="reguler"
                                    ...
                                    <label class="text-center cursor-pointer mb-0 flex-grow-1 mx-1">
                                        <input type="radio" name="kat_radio" id="kategori_yes" value="member"
                                            class="sr-only peer">
                                        <div class="status-item">
                            ...
                            <div class="nik-form mb-4 hidden animate-in slide-in-from-top-2">
                                <div
                                    class="p-3 border-start border-4 border-danger bg-danger/5 position-relative rounded-end shadow-sm">
                                    <label class="form-label fw-black text-danger small uppercase tracking-tighter">Cari NIK
                                        / Nama Anggota</label>
                                    <input type="text" id="nik-input"
                                        class="form-control border-dark font-bold shadow-sm"
                                        placeholder="Ketik NIK atau Nama..." autocomplete="off">

                                    <div class="dropdown-menu w-100 shadow-lg" id="dropdown-member"
                                        style="max-height: 250px; overflow-y: auto; z-index: 1060;">
                                    </div>

                                    <div id="member-info"
                                        class="mt-2 p-2 bg-success text-white rounded text-xs font-bold hidden shadow-sm border border-white">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="metode_bayar" id="metode_bayar"
                                    class="form-select [&>option]:font-semibold border-dark fw-black text-primary py-2 shadow-sm"
                            ...
                            <div class="row g-2 mb-3">
                                <div class="col-6">
                                    <label id="nominal-label" class="text-[10px] font-black uppercase text-muted">Dibayar
                                        (Tunai)</label>
                                    <div class="input-group">
                                        <span class="input-group-text bg-white border-dark text-xs font-bold"
                                            style="border-radius: 10px 0 0 10px;">Rp</span>
                                        <input type="number" name="nominal" id="nominal"
                                            class="form-control border-dark input-bayar-custom text-danger shadow-sm"
                                            placeholder="0" style="border-radius: 0 10px 10px 0 !important;">
                                    </div>
                                    <p id="info-bill" class="text-[9px] text-danger mt-1 hidden font-bold text-uppercase">
                                        *Kosongkan jika Full BON</p>
                                </div>
                                <div class="col-6">
                                    <label id="label-ket"
                                        class="text-[10px] font-black uppercase text-muted">Kembalian</label>
                                    <div class="kalkulasi-box shadow-sm text-truncate" id="kalkulasi">Rp 0</div>
                                    <p id="info-ket-bill" class="text-[9px] text-danger mt-1 font-black hidden uppercase">
                                    </p>
                                </div>
                                <div id="quick_acces" class="flex justify-evenly items-center">
                                    <span id="qa_5"
                                        class="flex justify-center rounded-md py-1 border-slate-300 cursor-pointer border hover:border hover:border-sky-400 px-2 items-center text-white bg-orange-400 hover:bg-orange-500 active:bg-orange-600">
                                        <i class="fa-solid fa-money-bill-1 mr-1"></i>
                                        Rp.5.000
                                    </span>
                                    <span id="qa_10"
                                        class="flex justify-center rounded-md py-1 border-slate-300 cursor-pointer border hover:border hover:border-sky-400 px-2 items-center text-white bg-purple-400 hover:bg-purple-500 active:bg-purple-600">
                                        <i class="fa-solid fa-money-bill-1 mr-1"></i>
                                        Rp.10.000
                                    </span>
                                    <span id="qa_50"
                                        class="flex justify-center  rounded-md py-1 border-slate-300 cursor-pointer border hover:border hover:border-sky-400 px-2 items-center text-white bg-blue-400 hover:bg-blue-500 active:bg-blue-600">
                                        <i class="fa-solid fa-money-bill-1 mr-1"></i>
                                        Rp.50.000
                                    </span>
                                    <span id="qa_100"
                                        class="flex justify-center rounded-md py-1 border-slate-300 cursor-pointer border hover:border hover:border-sky-400 px-2 items-center text-white bg-red-400 hover:bg-red-500 active:bg-red-600">
                                        <i class="fa-solid fa-money-bill-1 mr-1"></i>
                                        Rp.100.000
                                    </span>
                                </div>
                            ...
                            <hr>
                            <button type="button"
                                class="btn btn-danger w-100 fw-black py-3 shadow-lg transform active:scale-95 transition-all mb-4"
                                id="btn-proses" style="border-radius: 12px; font-size: 1.1rem; letter-spacing: 1px;">
    ...
    <script>
        ...
        inputNik.addEventListener('input', function() {
            ...
            if (filtered.length > 0) {
                ...
                dropdownMember.classList.add('show');
                dropdownMember.innerHTML =
                    '<div class="p-3 text-center text-muted">Anggota tidak ditemukan...</div>';
            }
        });
        ...
        function pilihAnggota(m) {
            ...
            const simpananObj = simpananList.length > 0 ? simpananList.reduce((acc, curr) => {
                return {
                    saldo: acc.saldo + Number(curr.saldo)
                };
            }, {
                saldo: 0
            }) : null;
        ...
        function renderTable() {
            ...
            const isSimpanan = document.getElementById('metode_bayar').value === 'simpanan';

            if (isMember && isTunai || isSimpanan) {
                diskon = bruto * 0.1;
            }
        ...
        function KalkulasiNominal() {
            ...
            if (selisih >= 0) {
                ...
                dispKalkulasi.className =
                    'kalkulasi-box font-black text-center border-success text-success bg-white shadow-sm';
