...
<head>
    ...
    <style>
        body { font-family: 'Courier New', Courier, monospace; width: 75mm; margin: 0; padding: 5px; font-size: 12px; color: #000; }
        .text-center { text-align: center; }
        .divider { border-top: 1px dashed #000; margin: 8px 0; }
        table { width: 100%; border-collapse: collapse; }
        .item-name { display: block; font-weight: bold; text-transform: uppercase; margin-top: 5px; }
...
<body onload="window.print()">
    ...
    {{-- INFO DATA TRANSAKSI --}}
    <table>
        <tr><td width="30%">No</td><td>: {{ $transaksi->kode_transaksi }}</td></tr>
    ...
    @if(count($details) > 0)
        @foreach($details as $item)
            ...
            <div class="item-row">
                <span>{{ $item->qty }} x {{ number_format(($item->harga_satuan ?? ($item->subtotal / $item->qty)), 0, ',', '.') }}</span>
    ...
    <table style="font-weight: bold; font-size: 14px;">
        ...
        <tr>
            <td>TOTAL BAYAR</td>
            <td class="text-right">Rp {{ number_format(($transaksi->Nominal ?? $transaksi->grand_total), 0, ',', '.') }}</td>
        </tr>
    </table>
